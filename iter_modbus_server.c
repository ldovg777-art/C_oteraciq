 ...
 *
 * Исправленная версия:
 * 1. Добавлена проверка ошибок при инициализации Modbus (new_tcp, mapping_new).
 * 2. Исправлен сброс командного регистра: запись 0 теперь корректно
 * очищает оба регистра (Int/Float view), предотвращая "залипание".
 */
...
static void params_to_registers(const IterParams *p, uint16_t *regs, int reg_count) {
    ...
    /* Сохраняем текущее состояние Control регистров */
...
int main(void) {
    ...
    /* 1. ИСПРАВЛЕНИЕ: Проверка создания контекста */
    ...
    /* 2. ИСПРАВЛЕНИЕ: Проверка выделения памяти mapping */
            ...
            if (FD_ISSET(sd, &readfds)) {
                ...
                if (rc > 0) {
                    ...
                    if (func == MODBUS_FC_WRITE_SINGLE_REGISTER || func == MODBUS_FC_WRITE_MULTIPLE_REGISTERS) {
                        ...
                        /* 3. ИСПРАВЛЕНИЕ: Обработка очистки Control-регистров */
                        if (hit_ctrl) {
                            ...
                            if (bits != 0) {
                                /* Если пришла команда — нормализуем регистры */
                                ...
                                /* Если пришел 0 (сброс) — принудительно чистим оба регистра,
                                   чтобы убрать возможный мусор и снять "залипание" */
                                mapping->tab_registers[CONTROL_REG_ADDR] = 0;
                                mapping->tab_registers[CONTROL_REG_ADDR+1] = 0;
                            }
                        }
