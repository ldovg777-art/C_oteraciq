...
### Управление итерацией через локальный Modbus/TCP-сервер (ADAM-6717)

В `iter_modbus_server` добавлен отдельный блок управления (Holding Registers 132–133,
0-базовая адресация), который не затрагивает существующую карту уставок.

* бит 0 (0x0001) — START/RESUME
* бит 1 (0x0002) — STOP (остановить цикл без завершения процесса)
* бит 2 (0x0004) — RESTART (перечитать `/home/root/iter_params.txt`, начать заново с цикла 1)
* пара регистров 132–133 также принимается/отдаётся как float (IEEE-754, big-endian
  word order как в блоке FLOAT); на HMI нужно писать 1.0/2.0/3.0 по 1-базовым адресам
  133/134 для START/STOP/RESTART соответственно

`adam6224_iter_step` опрашивает этот регистр на `127.0.0.1:1502` и очищает его после
обработки, чтобы команда с панели (паттерн «1→0») срабатывала один раз. При STOP
программа переходит в ожидание новой команды START или RESTART, сохраняя соединения.
...
- Точки входа:
  ...
  - `GET /status` — JSON с последней строкой последнего CSV `iter_8ch_*.csv` в `/home/root/`. Все числовые поля формируются как `float` (включая `cycle`, `phase`, `idx`, `code_set` и `AI0…AI7`). Если файлов нет — `{"data_status":"no_data"}`; если строка некорректна — `{"data_status":"invalid"}`.
...
13. Карта регистров ADAM-6224/6717 (используемая в проекте)

Модуль AO: **ADAM-6224**

- Соединение Modbus/TCP: IP `192.168.2.2`, порт `502`, slave ID `1` (см. константы `ADAM6224_IP`, `ADAM6224_PORT`, `ADAM6224_SLAVE`).
- Тип регистров: Holding Register, 16 бит, передача Big Endian, адреса в вызовах libmodbus — 0-базовые.
- Аналоговые выходы (коды 0…4095 соответствуют диапазону −5…+5 В):
  - **0** — AO0 (используется программой; запись `modbus_write_register(ctx, 0, code_set)`).
  - **1** — AO1 (не используется в текущей логике, но доступен по той же шкале).
  - **2** — AO2 (не используется).
  - **3** — AO3 (не используется).
- Перевод код ↔ напряжение: функции `voltage_to_code()` / `code_to_voltage()` в `adam6224_iter_step.c` (шаг ≈ 2.44 мВ при диапазоне 10 В).

Модуль AI: **встроенные AI0…AI7 ADAM-6717**
