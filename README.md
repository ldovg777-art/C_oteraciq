...
Добавлен отдельный сервер `iter_modbus_server`, который поднимается на порту **1502** на самом 6717 и отображает `/home/root/iter_params.txt` в Holding Registers (float, порядок слов big-endian):

- 0–1: версия/протокол (1.0f).
- 2–3: `repeats` (0 = бесконечно, −1 = один проход, >0 = количество проходов).
- 4–5: `phases` (1..5).
- Фазы 1–5: по 12 регистров на фазу (6 float): `start_mV`, `end_mV`, `step_mV`, `period_ms`, `settle_ms`, `pause_ms`. Фаза 1 → регистры 6–17; фаза 2 → 18–29; ... фаза 5 → 54–65.

При записи через Modbus значения округляются до целых, ограничиваются по количеству фаз и сохраняются обратно в `iter_params.txt`; в ответ выдаются уже нормализованные данные.
