...
phase;idx;time_ms;iter_mV;iter_V;code_set;ao_V;AI0;AI1;AI2;AI3;AI4;AI5;AI6;AI7
...
phase;idx;time_ms;iter_mV;iter_V;code_set;ao_V;AI0;...;AI7
...
Начиная с текущей версии поддерживается массив фаз. Для каждой фазы задаётся набор:
...
Параметры **без префикса** относятся к первой фазе. Для последующих фаз используются ключи с индексом, например `step2_start_mV`, `step2_period_ms`, `step2_pause_ms`. Допустимы также префиксы `phaseN_`. Количество фаз определяется автоматически по максимальному индексу (либо можно задать `phases=N`). Значения по умолчанию для каждой фазы: start = −5000 мВ, end = 5000 мВ, step = 100 мВ, period = 100 мс, settle = 50 мс, pause = 0 мс.

Пример iter_params.txt с двумя фазами (симметричный профиль туда-обратно и пауза между ними):

```
# Фаза 1: от -5 В до 0 В
start_mV=-5000
end_mV=0
step_mV=250
period_ms=100
settle_ms=40
pause_ms=200   # пауза 200 мс перед стартом следующей фазы

# Фаза 2: от 0 В до +5 В (индексы начинаются с 1)
step2_start_mV=0
step2_end_mV=5000
step2_step_mV=250
step2_period_ms=150
step2_settle_ms=60
step2_pause_ms=0   # пауза после второй фазы не нужна
```
...
1;0;12.345;-5000;-5.000000;0;-5.000000;0.001234;0.001235;...;0.001240
