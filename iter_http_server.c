...
static int parse_double_field(const char *s, double *out)
{
    if (!s || !out)
        return -1;
...
static void handle_status(int client_fd)
{
    ...
    double cycle_v = 0.0, phase_v = 0.0, idx_v = 0.0;
    double time_ms_v = 0.0, iter_mV_v = 0.0, iter_V_v = 0.0;
    double code_set_v = 0.0, ao_V_v = 0.0;
    double ai_v[8] = {0};

    if (parse_double_field(fields[0], &cycle_v) != 0 ||
        parse_double_field(fields[1], &phase_v) != 0 ||
        parse_double_field(fields[2], &idx_v) != 0) {
        const char *body = "{\"data_status\":\"invalid\"}";
        send_simple_response(client_fd, 200, "OK", "application/json", body);
        return;
    }

    double *float_fields[] = {&time_ms_v, &iter_mV_v, &iter_V_v, &code_set_v,
                              &ao_V_v,     &ai_v[0],   &ai_v[1],  &ai_v[2],
                              &ai_v[3],     &ai_v[4],   &ai_v[5],  &ai_v[6],
                              &ai_v[7]};

    for (int i = 0; i < 13; i++) {
        if (parse_double_field(fields[i + 3], float_fields[i]) != 0) {
            const char *body = "{\"data_status\":\"invalid\"}";
            send_simple_response(client_fd, 200, "OK", "application/json", body);
            return;
        }
    ...
    snprintf(json, sizeof(json),
             ...
             "\"cycle\":%.1f,\"phase\":%.1f,\"idx\":%.1f,\"time_ms\":%.3f," \
             "\"iter_mV\":%.3f,\"iter_V\":%.6f,\"code_set\":%.1f,\"ao_V\":%.6f," \
             "\"AI\":[%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f]}",
             safe_path,
             cycle_v, phase_v, idx_v, time_ms_v,
             iter_mV_v, iter_V_v, code_set_v, ao_V_v,
             ai_v[0], ai_v[1], ai_v[2], ai_v[3], ai_v[4], ai_v[5], ai_v[6], ai_v[7]);
