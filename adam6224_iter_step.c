 ...
 * Версия v19:
 * - Добавлена возможность настройки Modbus TCP параметров через iter_params.txt.
 * - Параметры rs485_ip, rs485_port, rs485_slave (ADAM-6224) можно менять без перекомпиляции.
 * 
 * Версия v18:
 * - Добавлена автоматическая ротация логов stdout.
 * - Программа теперь сохраняет только текущий (неполный) и предыдущий (полный) циклы.
 * - Логи записываются в worker_out_current.log и worker_out_prev.log.
 * 
 * Версия v17:
 * - Исправлен баг "зависания" фильтра при изменении размера окна на лету.
 * - При уменьшении size буфер теперь корректно обрезается, убирая старые "мертвые" данные.
 */
...
static void update_filter(float val, float *buffer, int *count, int *idx, int size) {
    /* Санитизация входного размера */
    if (size < 1) { size = 1; }
    if (size > MAX_FILTER_SIZE) { size = MAX_FILTER_SIZE; }

    /* ФИКС: Если размер фильтра уменьшили на лету, обрезаем count */
    if (*count > size) {
        *count = size;
        /* Сбрасываем индекс, чтобы он не указывал за пределы массива */
        if (*idx >= size) { *idx = 0; }
    }

    /* Стандартная логика кольцевого буфера */
    if (*count < size) {
        buffer[(*count)++] = val;
    } else {
        buffer[*idx] = val;
...
int main(void) {
    ...
    if (ctx) { modbus_set_slave(ctx, par.rs485_slave); modbus_set_response_timeout(ctx, 2, 0); }
    int retry=0;
    while(!g_stop) {
    ...
    while (!g_stop) {
        ...
        filter_count_chem = 0; filter_idx_chem = 0;
        memset(filter_buffer_chem, 0, sizeof(filter_buffer_chem));
        filter_count_redox1 = 0; filter_idx_redox1 = 0;
        memset(filter_buffer_redox1, 0, sizeof(filter_buffer_redox1));
        filter_count_redox2 = 0; filter_idx_redox2 = 0;
        memset(filter_buffer_redox2, 0, sizeof(filter_buffer_redox2));
